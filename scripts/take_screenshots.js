...
async function takeScreenshots() {
  ...
  try {
    ...
    if (onLoginPage) {
      console.log(
        "Login API unavailable. Seeding local auth for screenshot capture..."
      );
    ...
    if (!fileInput) {
      ...
      await page.screenshot({
        path: path.join(IMAGES_DIR, "debug_no_file_input.png"),
      });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(IMAGES_DIR, "debug_analysis_timeout.png"),
      });
    ...
    await page.screenshot({
      path: path.join(IMAGES_DIR, "3_risk_weakness.png"),
    });

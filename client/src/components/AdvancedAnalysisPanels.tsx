            ...
            <div className="space-y-2 mb-4 mt-4">
              {data.map((d, i) => (
                ...
                >
                  ...
                  <span
                    className={`text-xs font-mono w-16 text-right ${d.netProfit >= 0 ? "text-[oklch(0.72_0.18_145)]" : "text-[oklch(0.65_0.2_20)]"}`}
                  >
                    {d.netProfit >= 0 ? "+" : ""}
                    {d.netProfit.toFixed(1)}
                  </span>
                </motion.div>
              ))}
            ...
            <div className="border-t border-border pt-3 space-y-2">
              ...
              {bestDay.winRate >= 60 && bestDay.trades >= 3 && (
                <div className="flex gap-2 p-2.5 rounded-lg bg-[oklch(0.15_0.04_145)] border border-[oklch(0.72_0.18_145/0.2)]">
                  <span className="text-[oklch(0.72_0.18_145)] text-xs shrink-0 mt-0.5">
                    ✓
                  </span>
...
export function LotSizeCorrelationPanel({ data }: { data: LotSizeGroup[] }) {
  ...
  const weightedWinRate =
    totalTrades > 0
      ...
      : 0;
  const avgPF =
    data.reduce((s, g) => s + Math.min(g.profitFactor, 10) * g.trades, 0) /
    (totalTrades || 1);

  // Detect specific problems
  const allGroupsLosing = data.every(g => g.netProfit < 0);
  const allGroupsLowWR = data
    .filter(g => g.trades >= 3)
    .every(g => g.winRate < 50);
  const allGroupsLowPF = data
    .filter(g => g.trades >= 3)
    .every(g => g.profitFactor < 1);
  const largeLotWorse =
    hasMultipleGroups && largeLot.winRate < smallLot.winRate - 10;
  const largeLotMuchWorsePF =
    hasMultipleGroups &&
    largeLot.trades >= 3 &&
    smallLot.trades >= 3 &&
    largeLot.profitFactor < smallLot.profitFactor * 0.7;
  const largeLotBiggerLoss =
    hasMultipleGroups &&
    largeLot.netProfit < 0 &&
    smallLot.netProfit < 0 &&
    Math.abs(largeLot.netProfit / (largeLot.trades || 1)) >
      Math.abs(smallLot.netProfit / (smallLot.trades || 1)) * 1.5;

  // Find worst and best performing groups
  const worstGroup = data
    .filter(g => g.trades >= 3)
    .reduce((a, b) => (a.profitFactor < b.profitFactor ? a : b), data[0]);
  const bestGroup = data
    .filter(g => g.trades >= 3)
    .reduce((a, b) => (a.profitFactor > b.profitFactor ? a : b), data[0]);
  ...
  if (allGroupsLosing || allGroupsLowPF) {
    ...
    hasMultipleGroups &&
    bestGroup.profitFactor > 1 &&
    worstGroup.profitFactor < 0.8 &&
    worstGroup.trades >= 3
  ) {
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {data.map((row, i) => (
                    ...
                    >
                      <td className="py-2.5 px-2 font-medium text-foreground text-xs">
                        {row.label}
                      </td>
...
export function TradeQualityPanel({ data }: { data: TradeQualityAnalysis }) {
  ...
  const patterns = [
    {
      detected: data.isKotsuKotsuDokan,
      label: ja ? "コツコツドカン" : "Small wins / Large losses",
      severity: "high" as const,
    },
    {
      detected: data.isLargeWinDependent,
      label: ja ? "大勝ち依存" : "Large win dependency",
      severity: "high" as const,
    },
    {
      detected: data.hasLateSL,
      label: ja ? "損切り遅れ" : "Late stop-loss",
      severity: "high" as const,
    },
    {
      detected: data.hasEarlyTP,
      label: ja ? "利確が早すぎ" : "Early take-profit",
              ...
              <div className="bg-[oklch(0.14_0.02_260)] rounded-lg p-3">
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between">
                    ...
                    <span className="text-xs font-mono text-[oklch(0.72_0.18_145)]">
                      +{data.avgWinAmount.toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[10px] text-muted-foreground">
              ...
              <div className="bg-[oklch(0.14_0.02_260)] rounded-lg p-3">
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between">
                    ...
                    <span className="text-xs font-mono text-[oklch(0.65_0.2_20)]">
                      -{data.avgLossAmount.toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[10px] text-muted-foreground">
            ...
            <div className="space-y-2 mb-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] text-muted-foreground">
                    {ja
                      ? "利益集中率（上位20%のトレードが総利益の何%を占めるか）"
                      : "Profit concentration (% of total profit from top 20% of trades)"}
                  </span>
                  <span
                    className={`text-xs font-mono ${data.profitConcentration > 70 ? "text-[oklch(0.65_0.2_20)]" : "text-muted-foreground"}`}
                  >
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] text-muted-foreground">
                    {ja
                      ? "損失集中率（上位20%のトレードが総損失の何%を占めるか）"
                      : "Loss concentration (% of total loss from top 20% of trades)"}
                  </span>
                  <span
                    className={`text-xs font-mono ${data.lossConcentration > 60 ? "text-[oklch(0.65_0.2_20)]" : "text-muted-foreground"}`}
                  >
...
export function WinLossDistributionPanel({
  data,
}: {
        ...
        {open && (
          ...
          >
            ...
            <div className="mt-3 flex items-center gap-3">
              ...
              <span
                className={`text-xs font-mono ${data.skewness >= 0 ? "text-[oklch(0.72_0.18_145)]" : "text-[oklch(0.65_0.2_20)]"}`}
              >
                {data.skewness >= 0 ? "+" : ""}
...
export function LowQualityTradeImpactPanel({
  data,
}: {
  data: LowQualityTradeImpact[];
}) {
  const { language } = useLanguage();
  const ja = language === "ja";
  const [open, setOpen] = useState(false);
  ...
  const bestScenarioForSummary = data.find(
        ...
        {open && (
          ...
          >
            ...
            <div className="space-y-2 mb-4">
              {data.map((scenario, i) => {
                const pfChange =
                  scenario.originalPF > 0
                    ? ((scenario.newPF - scenario.originalPF) /
                        scenario.originalPF) *
                      100
                    : 0;
                const netChange =
                  scenario.newNetProfit - scenario.originalNetProfit;
                  ...
                  >
                      ...
                      <span
                        className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${
                          pfChange > 20
                            ...
                            : "bg-[oklch(0.3_0.02_260)] text-muted-foreground"
                        }`}
                      >
                        PF {pfChange >= 0 ? "+" : ""}
                        {pfChange.toFixed(0)}%
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center">
                      <div>
                        ...
                        <p className="text-xs font-mono">
                          <span className="text-muted-foreground">
                            {scenario.originalPF >= 999
                              ? "999+"
                              : scenario.originalPF.toFixed(2)}
                          </span>
                      ...
                      <div>
                        ...
                        <p className="text-xs font-mono">
                          <span className="text-muted-foreground">
                            {scenario.originalExpectancy.toFixed(1)}
                          </span>
                          <span className="text-muted-foreground/50 mx-0.5">
                            →
                          </span>
                          <span className="text-[oklch(0.72_0.18_145)]">
                            {scenario.newExpectancy.toFixed(1)}
                          </span>
                        </p>
                      </div>
                      <div>
                        ...
                        <p
                          className={`text-xs font-mono ${netChange >= 0 ? "text-[oklch(0.72_0.18_145)]" : "text-[oklch(0.65_0.2_20)]"}`}
                        >
                          {netChange >= 0 ? "+" : ""}
                          {netChange.toFixed(1)}
                        </p>
              ...
              (() => {
                ...
                return (
                  <div className="border-t border-border pt-3">
                    <p className="text-[10px] text-muted-foreground uppercase mb-2">
                      {ja
                        ? "最悪トレード一覧（損益額が最も大きいマイナス順）"
                        : "Worst Trades (sorted by largest loss)"}
                    </p>
                      ...
                      {worstTrades.map((t, i) => (
                        ...
                        >
                          <span className="text-muted-foreground font-mono text-[10px] w-16 shrink-0">
                            {t.time.toLocaleDateString(ja ? "ja-JP" : "en-US", {
                              month: "short",
                              day: "numeric",
                            })}
                          </span>
                          ...
                          <span className="text-muted-foreground/60 font-mono text-[10px] w-10 shrink-0">
                            ...
                            {t.time.getMinutes().toString().padStart(2, "0")}
                          </span>
                          ...
                          <span className="text-[oklch(0.65_0.2_20)] font-mono text-[10px] w-20 text-right shrink-0">
                            {t.profit.toFixed(2)}
                          </span>
                        </div>
                      ))}
            ...
            {data.some(s => s.originalPF > 0 && s.newPF > s.originalPF * 1.3) &&
              (() => {
                ...
                if (ja) {
                  if (p.dominantSymbol)
                    patternParts.push(
                      `通貨ペア「${p.dominantSymbol.symbol}」が${p.dominantSymbol.count}/${p.dominantSymbol.total}件`
                    );
                  if (p.dominantDay)
                    patternParts.push(
                      `${p.dominantDay.day}曜日が${p.dominantDay.count}/${p.dominantDay.total}件`
                    );
                  if (p.dominantHour)
                    patternParts.push(
                      `${p.dominantHour.hour}時台が${p.dominantHour.count}/${p.dominantHour.total}件`
                    );
                } else {
                  if (p.dominantSymbol)
                    patternParts.push(
                  ...
                  if (p.dominantHour)
                    patternParts.push(
                      `${p.dominantHour.hour}:00 hour: ${p.dominantHour.count}/${p.dominantHour.total}`
                    );
                }
